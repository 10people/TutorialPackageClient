        ...
        void QingGongTrigger()
        {
            ...
            foreach (KeyValuePair<string, Obj> objPair in ObjPools)
            {
                if (objPair.Value.ObjType != Games.GlobeDefine.GameDefine_Globe.OBJ_TYPE.OBJ_OTHER_PLAYER
                    && objPair.Value.ObjType != Games.GlobeDefine.GameDefine_Globe.OBJ_TYPE.OBJ_MAIN_PLAYER)
                {
                    continue;
                }
                ...
                if (null != objChar && objChar.IsLightState == true)
                {
                    continue;
                }
                ...
                if (Vector3.Distance(m_PointTransform.position, objPair.Value.ObjTransform.position) < m_TriggerRadius)
                {
                    ...
                    m_TriggerEvent.EventID = Games.GlobeDefine.GameDefine_Globe.EVENT_DEFINE.EVENT_QINGGONG;
                    ...
                    if (objPair.Value.ObjType == Games.GlobeDefine.GameDefine_Globe.OBJ_TYPE.OBJ_MAIN_PLAYER &&
