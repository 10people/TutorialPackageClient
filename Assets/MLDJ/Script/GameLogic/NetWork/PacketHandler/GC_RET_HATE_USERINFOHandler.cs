...
{
    public class GC_RET_HATE_USERINFOHandler : Ipacket
    {
        public uint Execute(PacketDistributed ipacket)
        {
            GC_RET_HATE_USERINFO packet = (GC_RET_HATE_USERINFO)ipacket;
            ...
            if (null != GameManager.gameManager.PlayerDataPool.HateList)
            {
                for (int i = 0; i < packet.guidCount; ++i)
                {
                    ...
                    GameManager.gameManager.PlayerDataPool.HateList.UpdateRelationUserInfo(_relation);
                }
            }
