...
public class LastSpeakerChatLogic : MonoBehaviour {
    ...
    private List<UInt64> m_GuildMemberSpeak = new List<UInt64>();

    private const int m_professionCount = 4;
    public UISprite[] ProfessionSpritesMe;
    public UILabel m_otherPlayerLevel;
    void Awake()
    {
        m_Instance = this;
    }
    ...
    public void OnReceiveChat()
    {
        ...
        if (m_ChatInfoLogic.CurChannelType == ChatInfoLogic.CHANNEL_TYPE.CHAT_TYPE_GUILD)
        {
            // 成员列表排序 最近发言的提前显示
            if (LastHistory.EChannel == GC_CHAT.CHATTYPE.CHAT_TYPE_GUILD)
            {
                AddLastSpeakers(LastHistory);
            }            
        }
    ...
    public void SetOtherPlayerInfo(int profession = 0, int level = 1, string speakerName = "")
    {
        for (int i = 0; i < m_professionCount && i < ProfessionSpritesMe.Length; i++)
        {
            ProfessionSpritesMe[i].gameObject.SetActive(profession == i);
        }
        m_SpeakerNameLabel.text = speakerName;
        m_otherPlayerLevel.text = level.ToString();
    }
...
//         }
    ...
    void AddFriend()
    {
		...
		/*if (GameManager.gameManager.PlayerDataPool.BlackList.IsExist(m_CurSpeakerGUID))
        {
    ...
    void ShieldPlayer()
    {
        ...
        if (GameManager.gameManager.PlayerDataPool.BlackList.IsExist(m_CurSpeakerGUID))
        {
            Singleton<ObjManager>.GetInstance().MainPlayer.SendNoticMsg(false, "#{11188}");
            return;
        }
    ...
    void OnReport()
    {
        MessageBoxLogic.OpenOKCancelBox(StrDictionary.GetClientDictionaryString("#{5452}"), "", OnReportOK, OnReportCancel);
    }
    ...
    {        
        if (Singleton<ObjManager>.GetInstance().MainPlayer != null)
        {
            ...
            {
                if (listChatHistory[i].SenderGuid == m_CurSpeakerGUID && !string.IsNullOrEmpty(listChatHistory[i].ChatInfo))
                {
                    szContent = listChatHistory[i].ChatInfo;            
                    break;
                }
            }
            StartCoroutine(Singleton<ObjManager>.GetInstance().MainPlayer.SendComplain(m_CurSpeakerGUID, m_CurSpeakerName, szContent, 1));
        }
    }
