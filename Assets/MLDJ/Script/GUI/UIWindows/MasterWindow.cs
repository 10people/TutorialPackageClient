...
public class MasterWindow : MonoBehaviour 
{
    ...
    void OnEnable()
    {
        m_MasterNameInput.defaultText = StrDictionary.GetClientDictionaryString("#{3637}");
    ...
    {
        UpdateData();
        if (tableButton.name == "6MasterShopTab")
        {
            m_nQingYiItemID = -1;
...
#region 商店
    ...
    public void OnClickCreateMaster()
    {
        ...
        if (curNoticeCharNum > GlobeVar.MAX_MASTER_NOTICE)
        {
            ...
            Singleton<ObjManager>.GetInstance().MainPlayer.SendNoticMsg(false, StrDictionary.GetClientDictionaryString("#{2798}"));
            return;
        }
        ...
        if (null == Utils.GetStrFilter(m_MasterNameInput.value, (int)Games.GlobeDefine.GameDefine_Globe.STRFILTER_TYPE.STRFILTER_NAME) &&
            null == Utils.GetStrFilter(m_MasterNoticeInput.value, (int)Games.GlobeDefine.GameDefine_Globe.STRFILTER_TYPE.STRFILTER_NAME))
        {
    ...
    void OnClickSkillActive_OK()
    {
        ...
        if (null == Utils.GetStrFilter(m_SkillNameInput.value, (int)Games.GlobeDefine.GameDefine_Globe.STRFILTER_TYPE.STRFILTER_NAME))
        {
            m_SkillNameInputObject.gameObject.SetActive(false);
    ...
    void OnClickChat_Reserve()
    {
        ...
        //如果目标是自己也不发送加好友
        if (Singleton<ObjManager>.GetInstance().MainPlayer.GUID == m_CurSelectReserveMember)
        {
            return;
        }
    ...
    public void OnClickChangeNotice()
    {
        ...
        if (null == Utils.GetStrFilter(m_MasterChangeNotice.value, (int)Games.GlobeDefine.GameDefine_Globe.STRFILTER_TYPE.STRFILTER_NAME))
        {
    ...
    public void UpdateMasterReserverRemain()
    {
        //有师门 并 是掌门
        if (GameManager.gameManager.PlayerDataPool.IsHaveMaster() == true &&
            GameManager.gameManager.PlayerDataPool.IsMasterChief() == true &&
