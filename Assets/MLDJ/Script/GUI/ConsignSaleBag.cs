...
public class ConsignSaleBag : MonoBehaviour
{
    ...
    public const int ITEMOBJECT_WIDTH = 85;     //每个ItemObject的宽度
    ...
    public static bool isCanConsignSale(GameItem checkItemInfo,bool isChatLink =false)
    {
        ...
        if (itemclass != (int)ItemClass.EQUIP &&
            ...
            itemclass != (int)ItemClass.STRENGTHEN)
        {
            return false;
        }
        //装备只上护符和戒指
        ...
        //强化类 只上伙伴技能书
        if (itemclass == (int)ItemClass.STRENGTHEN)
        {
            if (itemSubclass != (int)StrengthenSubClass.FELLOW_SKILL)
            {
                return false;
            }
        }
    ...
    private void ShowBackPack_Other()
    {
        // 过滤物品
        GameItemContainer BackPack = GameManager.gameManager.PlayerDataPool.BackPack;
        List<GameItem> itemlist = FilterConsignSale(BackPack, (int)ItemClass.STRENGTHEN);
