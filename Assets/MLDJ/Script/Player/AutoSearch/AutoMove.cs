...
//    private bool m_bIsMove = false;                         //是否正在移动
    ...
    {
        if (Cutscene.CutsceneManager.Instance.IsPlayingCutscene) return;
...
//        m_bIsMove = false;
    ...
    /// <summary>
    /// 保证PosSerial 设置是当前有效的
    /// </summary>
    private bool IsRightPosSerial(int iPosSerial)
    {
        int iVlaue = iPosSerial - m_nCurPosSerial;

        if (iVlaue >= 1 || (m_nCurPosSerial > 90 && iPosSerial < 10))
        {
            return true;
        }
