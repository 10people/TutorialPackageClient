...
public class LoginData
{
    ...
    {
        public PlayerRoleData(ulong _guid, int _type, string _name, int _level, int _ModelVisualID, int _WeaponID, int _WeaponEffectGem)
        {
            guid = _guid;
            type = _type;
            name = _name;
    ...
    public static void UpdateLoginRoleInfo(ulong roleGUID)
    {
        ...
        foreach (PlayerRoleData curData in loginRoleList)
        {
            if (curData.guid == roleGUID)
            {
                ...
                m_sbIsMale = (curData.type == (int)CharacterDefine.PROFESSION.DALI || curData.type == (int)CharacterDefine.PROFESSION.SHAOLIN || curData.type == (int)CharacterDefine.PROFESSION.GAIBANG) ? true : false;
    ...
    public static List<string> m_randomMaleNameList = new List<string>();
    public static List<string> m_randomFemaleNameList = new List<string>();
    ...
    public static void UpdateRandomNameList(GC_RET_RANDOMNAME packet)
    {
        if (packet.NSex == 1)
        {
            for (int i = 0, count = packet.namesCount; i < count; i++)
            {
                ...
                {
                    m_randomMaleNameList.Add(packet.GetNames(i));
                }
            }
        }else
        {
            for (int i = 0, count = packet.namesCount; i < count; i++)
            {
                if (packet.GetNames(i).Length > 0)
                {
                    m_randomFemaleNameList.Add(packet.GetNames(i));
                }
            }
        }
    }
