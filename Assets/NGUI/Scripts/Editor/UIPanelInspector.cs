...
// 		}
        ...
        int totalDCNum = 0;
        for (int pidx = 0; pidx < UIPanel.list.size; ++pidx)
        {
            UIPanel p = UIPanel.list[pidx];
            if (p != null && p.drawCalls != null)
            {
                totalDCNum += p.drawCalls.size;
            }
        }
        UIPanel curpanel = panel;
        for (int pidx = 0; pidx < UIPanel.list.size; ++pidx)
        {
            panel = UIPanel.list[pidx];

            //each panel begin

         
		for (int i = 0; i < panel.drawCalls.size; ++i)
		{
            UIDrawCall dc = panel.drawCalls[i];

            if (dc.panel != curpanel)
            {
...
// 				}
                ...
                //foreach (UIDrawCall drawcall in drawcalls)
                {
				    ...
				    for (int b = 0; b < panel.widgets.size; ++b)
				    {
                        UIWidget w = panel.widgets[b];
					    if (w != null && w.material == dc.material && w.depth == dc.depth)
						    ++count;
				    }
				    ...
				    if (sel != 0)
				    {
                        ...
                        for (int b = 0; b < panel.widgets.size; ++b)
					    {
						    ...
						    if (w != null && w.material == dc.material && w.depth == dc.depth && ++count == sel)
						    {
							    Selection.activeGameObject = w.gameObject;
							    break;
						    }
