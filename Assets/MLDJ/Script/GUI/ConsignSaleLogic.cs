...
public class ConsignSaleLogic : MonoBehaviour
{
   ...
   public GameObject m_QiTaGird;
   public UIGrid m_SubQiTaUIGrid;
   ...
   private int m_nCurBuyPage =0; //当前购买查看页
   ...
   private List<GameObject> m_ClickSprite = new List<GameObject>();
	...
	void Start () 
    {
...
#if BOBJOYSTICK
        //强制给打开
        m_SubWuQiGrid.SetActive(true);
        m_SubFangJuGrid.SetActive(true);
        m_SubShiPinGrid.SetActive(true);
        m_SubYaoPinGird.SetActive(true);
        m_SubCaiLiaoGird.SetActive(true);
#else
	    ...
	    if (m_bIsSearchForQiuGou ==false)
	    {
            m_KeyWordInput.label.text = StrDictionary.GetClientDictionaryString("#{4490}");
	    }
        //默认打开点击购买分页
	    ClickBuyBt();
...
#if BOBJOYSTICK
        UIRootJoyControllerHelper.Instance.setState(UIRootJoyControllerHelper.JoyUIState.JISHOU_UI);
#endif
	}
    ...
    void CleanBuyItemInfo()
    {
        ...
        {
            CurClickBuyItem.m_BakSprite.spriteName = "CommonBack6Nor";
    ...
    void OnLoadSearchClassBtItem(GameObject resObj, object param)
    {
        ...
        //其他按钮
        _gameObject = Utils.BindObjToParent(resObj, m_searchGird, "50QiTaBt");
        if (null != _gameObject)
            SetClassBtInfo(_gameObject, StrDictionary.GetClientDictionaryString("#{5223}"), (int)ConsignSale_SearchClass.OTHER, m_QiTaGird);
    ...
    void OnLoadSearchSubClassBtItem(GameObject resObj, object param)
    {
        ...
        _gameObject = Utils.BindObjToParent(resObj, m_SubWuQiGrid, "4Chu");
        if (null != _gameObject)
            SetSubClassBtInfo(_gameObject, StrDictionary.GetClientDictionaryString("#{5293}"), (int)ConsignSale_SearchClass.WEAPON, (int)ConsignSale_SearchSubClass.WEAPON_GAIBANG);
