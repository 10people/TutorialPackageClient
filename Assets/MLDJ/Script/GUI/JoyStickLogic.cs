...
public class JoyStickLogic : MonoBehaviour
{
    ...
    public UISprite m_BackGround1;                                            // 背景图1
    public UISprite m_BackGround2;                                           // 背景图2
    ...
    {
        get { return m_NewPlayerStep; }
        set { m_NewPlayerStep = value; }
    }
    void Awake()
    {
        m_Instance = this;
    ...
    void Update()
    {
            ...
            && !Singleton<ObjManager>.Instance.MainPlayer.QingGongState)
        {
			...
			if (!Singleton<ObjManager>.Instance.MainPlayer.IsDie() && !UIManager.IsSubUIShow() && (NewPlayerGuidLogic.IsOpenFlag == false || NewPlayerGuidLogic.CurrentGuideID != 1))
            {
...
#if UNITY_EDITOR
		if (!Singleton<ObjManager>.GetInstance().MainPlayer.IsDie() && !UIManager.IsSubUIShow() && !Singleton<ObjManager>.Instance.MainPlayer.IsInModelStory)
        {
...
//         m_BackGround1.gameObject.GetComponent<Spin>().enabled = false;
    ...
    void SendMoveDirection()
    {
        ...
        DoSendMoveDirection(nPosY, nPosX);
    ...
    private void DoSendMoveDirection(float nPosY, float nPosX)
    {
