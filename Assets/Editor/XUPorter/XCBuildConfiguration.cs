...
namespace UnityEditor.XCodeEditor
{
	public class XCBuildConfiguration : PBXObject
	{
		...
		protected const string OTHER_LINK_FLAGS_KEY = "OTHER_LDFLAGS";
		protected const string ENABLE_OBJC_EXCEPTION = "GCC_ENABLE_OBJC_EXCEPTIONS";
		...
		public bool AddOtherLinkFlags(PBXList flags)
		{
			bool modified = false;
			...
			foreach(string flag in flags)
			{
				if (!((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey(OTHER_LINK_FLAGS_KEY))
				{
					((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add(OTHER_LINK_FLAGS_KEY, new PBXList());
				}
				else if (((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY] is string)
				{
					string tempString = (string)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY];
					((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY] = new PBXList();
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY]).Add(tempString);
				}

				if (!((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY]).Contains(flag))
				{
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[OTHER_LINK_FLAGS_KEY]).Add( flag );
					modified = true;
				}
			}
		...
		public bool EnableObjCException(PBXList flags)
		{
			bool modified = false;
			...
			foreach(string flag in flags)
			{
				if (!((PBXDictionary)_data[BUILDSETTINGS_KEY]).ContainsKey(ENABLE_OBJC_EXCEPTION))
				{
					((PBXDictionary)_data[BUILDSETTINGS_KEY]).Add(ENABLE_OBJC_EXCEPTION, new PBXList());
				}
				else if (((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION] is string)
				{
					string tempString = (string)((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION];
					((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION] = new PBXList();
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION]).Add(tempString);
				}
				
				if (!((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION]).Contains(flag))
				{
					((PBXList)((PBXDictionary)_data[BUILDSETTINGS_KEY])[ENABLE_OBJC_EXCEPTION]).Add( flag );
					modified = true;
				}
			}
