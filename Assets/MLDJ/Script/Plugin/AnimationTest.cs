...
public class AnimationTest : MonoBehaviour
{
	...
	void PlayOtherAnim(int index)
    {
        ...
        if (animObjs[index] == null || animObjs[index].GetComponent<Animation>() == null)
        {
            return;
        }

        if (null != animObjs[index].GetComponent<Animation>() && index < animNames[index].Length)
            animObjs[index].GetComponent<Animation>().Play(animNames[index]);
    }
}
