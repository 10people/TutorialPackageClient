/********************************************************************************
 *	文件名：	CameraStateChangeToStareTarget.cs
 *	全路径：	\Script\Player\Controller\Camera\CameraStateChangeToStareTarget.cs
 *	创建人：    陈惟楚
 *	创建时间：   2015-03-20
 *
 *	功能说明： 移动摄像机以盯着某一个Obj
...
public class CameraStateChangeToStareTarget : ICameraState
{
    #region Fields
    ...
    #endregion
    #region Methods
    private CameraStateChangeToStareTarget() {}
    public CameraStateChangeToStareTarget(ThirdPersonCamera thirdPersonCamera)
    {
    ...
    public void UpdateCamera()
    {
        ...
        if (t > 1)
        {
            LookTarget(m_Target, null, 0);
            if (m_OnFinish != null) m_OnFinish();
            return;
        }
        t = iTween.Easing(0, 1, t, m_EaseType);
        Vector3 tempTargetPos = m_OldPos * (1 - t) + m_TargetTransformCache.position * t;
