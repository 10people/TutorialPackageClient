...
//             float test = 1/32;
//             Debug.Log(Mathf.CeilToInt(test));
//             float.TryParse(text, out test);
// 
//             Debug.Log("test " + test + " " + Mathf.CeilToInt(test));
            ...
            if (!string.IsNullOrEmpty(textCopy))
            {
                ...
                {
                    if (text.Length > 6 && text.Substring(2,5) == "showg")
                    {
                        ChatInfoLogic.Instance().ClearCurInput();
                        mInput.selected = false;
                        ChatInfoLogic.Instance().OnCloseClick();
    ...
    public void SubmitTextAndVoice(string text, byte[] voicechat)
    {
        if (!string.IsNullOrEmpty(text))
        {
            ...
            ChatHistoryItem item = new ChatHistoryItem();
            item.CleanUp();
            Utils.SendCGChatPak(text, item, voicechat);

            LogModule.DebugLog("SendCGChatPak");
        }
    ...
    public static void  SubmitVoice(string text,byte[] voicechat)
    {
        if (!string.IsNullOrEmpty(text))
        {
            text = Utils.StrFilter_Chat(text);
