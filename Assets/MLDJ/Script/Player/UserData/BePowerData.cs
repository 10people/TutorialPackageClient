...
public class BePowerData
{
    ...
    public enum BePowerWayDefine
    {
        ...
        EBPWD_MISSION_DAY=19,
        EDPWD_BIANSHEN_SKILLUP=20,
        EBPWD_WAY_MAX
    }
    ...
    //add by mawenbin
    //由于BePowerWindow.MemberListGrid使用异步加载逻辑，直接使用m_TabController.ChangeTab("Tab1")，可能会导致未知的逻辑问题；
    //添加此函数用于从微型复活界面进入正常复活界面时初始化PowerPushReliveListItem
    public static void ShowBePowerItemList()
    {
        if (null != delegateShowBePowerItemList)
        {
            delegateShowBePowerItemList();
        }
    }
