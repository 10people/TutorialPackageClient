...
public class UIDraggablePanel : MonoBehaviour
{
    ...
    public delegate void PanelDragDelegate(Vector2 offset);
    public PanelDragDelegate OnPanelDrag;
	...
	public void SetDragAmount (float x, float y, bool updateScrollbars)
	{
		...
		cr.y = oy;
		// 如有问题再改回来。。
/*		if(Mathf.Abs(mPanel.clipRange.x)  < Mathf.Abs(cr.x)   )*/
		    mPanel.clipRange = cr;
