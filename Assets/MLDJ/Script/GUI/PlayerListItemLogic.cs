...
public class PlayerListItemLogic : MonoBehaviour 
{
   ...
   // public UILabel m_PlayerTeamJobLabel;            // 队长/队员
    ...
    public UISprite m_PlayerHeadPic;                //头像
    ...
    static private Color m_OtherColor = new Color(180f / 255.0f, 1.0f, 209f / 255.0f, 1.0f);
    ...
    //显示好友
	public Relation m_relationFriend = null;
    public void InitPlayerListItemInfoFriend(Relation relation)
    {
        HideAllUI();

		m_relationFriend = relation;
    ...
    //显示黑名单
	public Relation m_relationBlack = null;
    public void InitPlayerListItemInfoBlack(Relation relation)
    {
        HideAllUI();

		m_relationBlack = relation;
	...
	public Relation m_relationHate = null;
    ...
    void SetPro(int nProfession)
    {
        ...
        // 玩家职业
       // m_PlayerProLabel.text = Utils.GetProfessionNameByType(nProfession);//StrDictionary.GetClientDictionaryString("#{" + CharacterDefine.PROFESSION_DICNUM[nProfession].ToString() + "}");

		//if (nProfession) 
		{
			m_PlayerProLabel.enabled = true;
            m_PlayerProLabel.spriteName = Utils.GetProfessionIcon(nProfession);
    ...
    void SetHeadPic(int nProfession)
    {
        ...
        m_PlayerHeadPic.gameObject.SetActive(true);
        //判断职业是否合法
        if (nProfession < 0 || nProfession >= (int)CharacterDefine.PROFESSION.MAX)
        {
            //非法职业ID，则强制为0
	...
	//=============================================
	// 好友功能 
	public UIButton m_chatPrivate ;     // 私聊
	public UIButton m_requestTeam ;  // 邀请组队

	// 黑名单
	public UIButton m_checkAllInfo ;   		// 查看详情
	public UIButton m_deletBtn_black  ; // 黑名单删除

	//死敌
	public UIButton m_deletBtn_Hite ;
	public UIButton m_fellowBtn_Hite ;
	
	// 公用
	public UIButton m_otherBtn ;

	// 批量删除
	public UIToggle m_toggleBtn ;
	...
	public void onDelete()
	{
		//if( m_ItemType  ==  PlayerListItemType.FriendInfo)
		{
			string dicStr = "" ;
			if (m_ItemType == PlayerListItemType.FriendInfo)
				dicStr = StrDictionary.GetClientDictionaryString("#{5251}");
			else if (m_ItemType == PlayerListItemType.BlackPlayerInfo)
				dicStr = StrDictionary.GetClientDictionaryString("#{10838}");
			else if (m_ItemType == PlayerListItemType.HatePlayerInfo)
				dicStr = StrDictionary.GetClientDictionaryString("#{10837}");
	...
	public void DelFriendOK()
	{
		...
		if (m_ItemType == PlayerListItemType.Invalid || GlobeVar.INVALID_GUID == m_Guid) {
						return;
				}

		// 删除好友
		else if (m_ItemType == PlayerListItemType.FriendInfo) {
						if (GameManager.gameManager.PlayerDataPool.FriendList.IsExist (m_Guid)) {
								Singleton<ObjManager>.GetInstance ().MainPlayer.ReqDelFriend (m_Guid);
						}
				}
	...
	private void _hideAllUIBtn()
	{
		m_chatPrivate.gameObject.SetActive (false);
		m_requestTeam .gameObject.SetActive (false);
		m_checkAllInfo.gameObject.SetActive (false);
		m_deletBtn_black.gameObject.SetActive (false);
		m_deletBtn_Hite.gameObject.SetActive (false);
		m_fellowBtn_Hite.gameObject.SetActive (false);
		m_otherBtn.gameObject.SetActive (false);
		m_toggleBtn.gameObject.SetActive (false);
	}

	private void _showBtnByType()
	{
		_hideAllUIBtn ();

		switch (m_ItemType) 
		{
		case PlayerListItemType.FriendInfo :
			m_chatPrivate.gameObject.SetActive (true);
			m_requestTeam.gameObject.SetActive (true);
			m_otherBtn.gameObject.SetActive (true);// hide temporarily
			break ;
		case PlayerListItemType.BlackPlayerInfo :
			m_deletBtn_black.gameObject.SetActive (true);
			m_checkAllInfo.gameObject.SetActive (true);
			break ;
		case PlayerListItemType.HatePlayerInfo:
			m_deletBtn_Hite.gameObject.SetActive (true);
			m_fellowBtn_Hite.gameObject.SetActive (true);
			m_otherBtn.gameObject.SetActive (false);// hide temporarily
			break ;
		}
	}

	public void showDeleteAllToggleBtn()
	{
		_hideAllUIBtn ();
		m_toggleBtn.gameObject.SetActive (true);
	}

	public void hideDeleteAllToggleBtn()
	{
		_showBtnByType ();

		m_toggleBtn.value = false;
		toggltDelGuid ();
	}
	...
	{
		if (GlobeVar.INVALID_GUID != m_Guid && m_PlayerName != "")
		{
			//发起私聊
			//未打开过则创建
			if (null == ChatInfoLogic.Instance())
