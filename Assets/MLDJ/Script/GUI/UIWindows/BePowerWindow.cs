...
public class BePowerWindow : UIControllerBase<BePowerWindow>
{
    ...
    void SetDefaultLeftLevel()
    {
        for (int i = 0; i < m_PowerLeftItem.Length; i++ )
        {
            ...
            {
                m_PowerLeftItem[i].SetData(i+1,0,0, (i == 0));
            }
        }
    }
    ...
    void SetSlider(int level)
    {
        ...
        for (int i = 0; i < SpriteLevelIcon.Length; i++)
        {
            ...
            Transform  highLightTransform = SpriteLevelIcon[i].gameObject.transform.FindChild("HighLight");
            if (null != highLightTransform && null != highLightTransform.gameObject)
            {
                highLightTransform.gameObject.SetActive(activeThis);
            }
        }
    }
