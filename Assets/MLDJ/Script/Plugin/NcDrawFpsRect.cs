...
public class NcDrawFpsRect : MonoBehaviour
{
	...
	public	Rect		startRect		= new Rect( 0, 0, 100, 150 );	// The rect the window is initially displayed at.
...
//#endif
	...
	void DoMyWindow(int windowID)
    {
        ...
        //GUI.Label(new Rect(0, 20, startRect.width, startRect.height), (int)NetWorkLogic.GetMe().GetConnectStautus() + ":" + NetManager.m_IsAskConnecting, style);
        //GUI.Label(new Rect(0, 20, startRect.width, startRect.height), ActiveScene.IsMoveRay + "", style);


        if (Singleton<ObjManager>.GetInstance().MainPlayer != null)
        {
            tempNetSpeed = Singleton<ObjManager>.GetInstance().MainPlayer.NetSpeed;
            if (tempNetSpeed < 0) tempNetSpeed = 0;

            GUI.Label(new Rect(0, 20, startRect.width, startRect.height),
                (tempNetSpeed + 250).ToString("f0") + ":" + tempNetSpeed.ToString("f0"), style);
        }
