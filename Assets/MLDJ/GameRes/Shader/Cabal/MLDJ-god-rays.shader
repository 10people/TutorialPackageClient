...
Shader "Cabal/Blinking GodRays" {

Properties {
	_MainTex ("Base texture", 2D) = "white" {}
	_FadeOutDistNear ("Near fadeout dist", float) = 10.0	
	_FadeOutDistFar ("Far fadeout dist", float) = 10000.0	
	_Multiplier("Color multiplier", float) = 1.0
	_Bias("Bias",float) = 0.0
	_TimeOnDuration("ON duration",float) = 0.5
	_TimeOffDuration("OFF duration",float) = 0.5
	_BlinkingTimeOffsScale("Blinking time offset scale (seconds)",float) = 5.0
	_SizeGrowStartDist("Size grow start dist",float) = 5.0
	_SizeGrowEndDist("Size grow end dist",float) = 50
	_MaxGrowSize("Max grow size",float) = 2.5
	_NoiseAmount("Noise amount (when zero, pulse wave is used)", Range(0,0.5)) = 0.0
