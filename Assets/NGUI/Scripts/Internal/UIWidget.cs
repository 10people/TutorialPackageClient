...
public abstract class UIWidget : MonoBehaviour
{
    ...
    public float alpha
    {
        ...
        set
        {
            Color c = mColor; c.a = value; color = c; 

			//alpha值改变后需要删除原来的DrawCall，与depth同理
            if (null != mPanel)
            {
                mPanel.RemoveDrawCall(material, mDepth);

                //Remove掉DrawCall后需要通知Panel重建新的DrawCall
                mPanel.MarkMtlChanged(material);
            }
        }
    }
...
// 	}
		...
		if (material != null && p != null)
		{
			...
			if (mChanged)
			{
				...
				else
				{
                    ...
                    // 如果Widget从可见变成不可见，需要清除原有的DrawCall，减少DrawCall数量
                    // 否则由于Widget不可见，无法更新VertexBuffer，原有DrawCall的信息是错误的
                    if (panel)
                    {
                        panel.RemoveDrawCall(material, depth);
                        panel.MarkMtlChanged(material);
                    }
    ...
    /// <summary>
    /// Virtual function, get the type of this widget
    /// </summary>
    /// <returns>WidgetType</returns>
    /// 
    virtual public WidgetType GetWidgetType() { return WidgetType.WT_Unknown; }
