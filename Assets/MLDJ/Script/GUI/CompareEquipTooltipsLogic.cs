...
public class CompareEquipTooltipsLogic : MonoBehaviour {
    ...
    public BoxCollider m_CompareDescBoxCollider;
    ...
    void ShowCompareName(GameItem equip)
    {
        ...
        m_CompareNameLabel.text += Utils.GetEquipAttrFactorName(equip);
        ...
        if (nExistTime > 0)
        {
            m_CompareNameLabel.text += StrDictionary.GetClientDictionaryString("#{3316}", (float)nExistTime / 60f / 24f);
        }
    }
    ...
    void ShowCompareLevel(GameItem equip)
    {
        ...
        m_CompareLevelLabel.text += StrDictionary.GetClientDictionaryString("#{3317}", nCompareLevel);
    }
    ...
    void ShowCompareDesc(GameItem equip)
    {
        ...
        if (tabItem != null)
        {
            string strCompareDesc = tabItem.Tips;
            strCompareDesc = strCompareDesc.Replace("#r", "\n");
            m_CompareDescLabel.text += strCompareDesc;
            m_CompareDescBoxCollider.size = new Vector3(m_CompareDescLabel.width, m_CompareDescLabel.printedSize.y, 0);
            m_CompareDescBoxCollider.center = new Vector3(0.5f * m_CompareDescBoxCollider.size.x, -0.5f * m_CompareDescBoxCollider.size.y, 0);
        }       
    }
