...
public class ResourceManager : MonoBehaviour
{
    ...
    public static GameObject LoadPlayVoiceBtn(GameObject BtnParent)
    {
        string strPath = "Prefab/UI/PlayVoiceBtn";
        string strPrefabName = "PlayVoiceBtn";
        GameObject BtnItem = InstantiateResource(strPath, strPrefabName) as GameObject;
        if (null != BtnItem)
        {
            if (null != BtnParent)
                BtnItem.transform.parent = BtnParent.transform;
            BtnItem.transform.localPosition = Vector3.zero;
            BtnItem.transform.localScale = Vector3.one;
            BtnItem.name = strPrefabName;
        }
        return BtnItem;
    }
