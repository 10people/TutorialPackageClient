...
public class UICamera : MonoBehaviour
{
    ...
    {
        get { return m_TouchFingerID; }
        set { m_TouchFingerID = value; }
    } 
    ...
    static public UICamera eventHandler
    {
        get
        {
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                UICamera cam = list[i];
                if (cam == null || !cam.enabled || !NGUITools.GetActive(cam.gameObject) || cam.gameObject.name == "Camera-JoyStick") continue;
                return cam;
            }
            return null;
        }
 ...
 || Application.platform == RuntimePlatform.WP8Player
    ...
    void Update()
    {
        if (!Singleton<ObjManager>.Instance.MainPlayer ||
            (Singleton<ObjManager>.Instance.MainPlayer && !Singleton<ObjManager>.Instance.MainPlayer.IsInModelStory))
        {
