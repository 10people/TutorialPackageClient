...
class PushNotification
{
    ...
    public static void addPushNotificationInfo(string news, int data,int RepeatType)
    {
		...
		//LogModule.DebugLog("addPushNotificationInfo" + "info.m_szNews:" + info.m_szNews.ToString() + "info.m_nData:" + info.m_nData.ToString() + "info.m_nRepeatType:" + info.m_nRepeatType.ToString());
    }
...
//                      + "PushNotificationInfoList[i].m_nRepeatType:" + PushNotificationInfoList[i].m_nRepeatType.ToString());
    ...
    {
        #if UNITY_IPHONE 
        UnityEngine.iOS.LocalNotification Notifcation = new UnityEngine.iOS.LocalNotification();
        Notifcation.applicationIconBadgeNumber = -1;
        UnityEngine.iOS.NotificationServices.PresentLocalNotificationNow(Notifcation);
