...
public class UIDrawCall : MonoBehaviour
{
    ...
    //Max drawCall in a panel
    const int mDrawCallLimit = 200;

    //depth used for finding and sorting
    int mDepth = 0;
	...
	public int renderQueue
	{
		...
		set
		{
			...
			{
                //panel内最大DrawCall(widget数目最好也不要超过)为mDrawCallLimit，
                //用Panel.index * mDrawCallLimit相当于
                //先按Panel深度排序，再按Panel内的Widget深度排序
                mMat.renderQueue = mSharedMat.renderQueue + value + panel.index * mDrawCallLimit;
#if UNITY_EDITOR
    ...
    public Material dynamicMat { get { return mMat; } set { mMat = value; } }
	...
	public void RebuildMaterial ()
	{
        ...
        mMat.renderQueue = mSharedMat.renderQueue + mRenderQueue + panel.index * mDrawCallLimit;
	}
    ...
    /// Compare.
    /// </summary>
    static public int CompareFunc(UIDrawCall left, UIDrawCall right)
    {
