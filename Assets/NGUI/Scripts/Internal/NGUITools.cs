...
static public class NGUITools
{
	...
	{
        // é…ç½®é¡¹åå­—å†™åäº† å®žé™…ä¸Šæ„æ€æ˜¯å…³é—­éŸ³æ•ˆ
	...
	static public int ParseSymbol (string text, int index, List<Color> colors, bool premultiply)
	{
		...
		if (index + 2 < length)
		{
			...
			{
				if (text[index + 7] == ']' &&
                    ((text[index + 1] >= 'a' && text[index + 1] <= 'z') || (text[index + 1] >= 'A' && text[index + 1] <= 'Z') || (text[index + 1] >= '0' && text[index + 1] <= '9')) &&
                    ((text[index + 2] >= 'a' && text[index + 2] <= 'z') || (text[index + 2] >= 'A' && text[index + 2] <= 'Z') || (text[index + 2] >= '0' && text[index + 2] <= '9')) &&
                    ((text[index + 3] >= 'a' && text[index + 3] <= 'z') || (text[index + 3] >= 'A' && text[index + 3] <= 'Z') || (text[index + 3] >= '0' && text[index + 3] <= '9')) &&
                    ((text[index + 4] >= 'a' && text[index + 4] <= 'z') || (text[index + 4] >= 'A' && text[index + 4] <= 'Z') || (text[index + 4] >= '0' && text[index + 4] <= '9')) &&
                    ((text[index + 5] >= 'a' && text[index + 5] <= 'z') || (text[index + 5] >= 'A' && text[index + 5] <= 'Z') || (text[index + 5] >= '0' && text[index + 5] <= '9')) &&
                    ((text[index + 6] >= 'a' && text[index + 6] <= 'z') || (text[index + 6] >= 'A' && text[index + 6] <= 'Z') || (text[index + 6] >= '0' && text[index + 6] <= '9')))
				{
					if (colors != null)
					{
