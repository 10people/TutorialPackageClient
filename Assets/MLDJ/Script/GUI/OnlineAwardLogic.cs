...
public class OnlineAwardLogic : MonoBehaviour
{
	...
	public float getSliderVlue()
	{
		int tem   = m_OnlineAwardID - 1 > 0 ? m_OnlineAwardID - 1 : 0;
	
		if( m_OnlineAwardID > 0  )
			return ((float)tem / 6 + (float)( 10 * 60 - m_LeftTime) / 3600);

		return 0;
	}
	public void UpdateSlider( float vlue )
	{
		sliderLoading.sliderValue = vlue; ;
	}
