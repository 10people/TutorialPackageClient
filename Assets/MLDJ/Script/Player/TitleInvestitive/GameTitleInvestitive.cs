...
namespace Games.TitleInvestitive
{
    ...
    {
        TITLE_NAME = 22,
        SYSTEMTITLE_NUM = 32,//系统程序支持存储的32个
    ...
    public struct GameSystemTitleInvestitive
    {
        ...
        public int GainTime
        {
            get { return m_GainTime; }
            set { m_GainTime = value; }
        }

        public GameSystemTitleInvestitive(int nTitleID, int nGainTime)
        {
            m_TitleID = nTitleID;
        ...
        public void Clear()
        {
            m_TitleID = GlobeVar.INVALID_ID;
            m_GainTime = GlobeVar.INVALID_ID;
        }
    ...
    public struct GameUserDefTitleInvestitive
    {
        ...
        public GameUserDefTitleInvestitive(int nTitleID, string strFullTitleName, int nGainTime)
        {
            m_TitleID = nTitleID;
            m_strFullTitleName = strFullTitleName;
            m_GainTime = nGainTime;
        }
    ...
    public class GameTitleInvestitive
    {
        ...
        public GameTitleInvestitive()
        {
            ...
            {
                m_SystemTitle[i] = new GameSystemTitleInvestitive(GlobeVar.INVALID_ID, GlobeVar.INVALID_ID);
            }
            for (int i = 0; i < (int)TITLE_SIZE.USERDEF_NUM; i++)
            {
                m_UserDefTitle[i] = new GameUserDefTitleInvestitive(GlobeVar.INVALID_ID, "", GlobeVar.INVALID_ID);
            }
            m_ActiveTitle = -1;
        }
        ...
        public void HandleGainTitle(int nTitleIndex, int nTitleID, int nGainTime, string strUserDef, int nActiveTitle)
        {
            if (0 <= nTitleIndex && nTitleIndex < (int)TITLE_SIZE.USERDEF_START)
            {
                m_SystemTitle[nTitleIndex].TitleID = nTitleID;
                m_SystemTitle[nTitleIndex].GainTime = nGainTime;
            }
            else if ((int)TITLE_SIZE.USERDEF_START <= nTitleIndex && nTitleIndex < (int)TITLE_SIZE.TITLE_TOTAL)
            {
                ...
                m_UserDefTitle[nTitleIndex - (int)TITLE_SIZE.USERDEF_START].GainTime = nGainTime;
