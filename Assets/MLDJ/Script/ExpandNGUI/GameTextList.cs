...
public class GameTextList : MonoBehaviour
{
    ...
    private int m_ChatCount = 0;
    private int NEEDCLEAR_COUNT = 500;
    ...
    protected void Add(string text, bool updateVisible, out int linesCount)
    {
        ...
        m_ChatCount += 1;
        if (m_ChatCount >= NEEDCLEAR_COUNT)
        {
            if (ChatInfoLogic.Instance() != null && false == ChatInfoLogic.Instance().WaitRefresh)
            {
