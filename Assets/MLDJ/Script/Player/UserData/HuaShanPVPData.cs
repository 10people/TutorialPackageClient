...
#region  world boss
    ...
    {
        public WorldBossTeamInfo(int _id, string _name, int _score, int _pos, int _cd, int _sceneid, int _instid)
        {
            id = _id;
            name = _name;
            score = _score;
    ...
    public static bool IsHuaShanPvPOpen()
    {
        ...
        if (curTime.DayOfWeek == DayOfWeek.Sunday)
        {
            if (curTime.Hour == 14)
            {
                if (curTime.Minute >= 40 && curTime.Minute <= 59)
                {
                    return true;
                }
            }
