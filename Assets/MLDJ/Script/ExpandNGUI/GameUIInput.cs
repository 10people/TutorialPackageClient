...
public class GameUIInput : UIWidgetContainer
{
    ...
    public bool selected
    {
        ...
        set
        {
            if (!value && gameObject != null && UICamera.selectedObject == gameObject) UICamera.selectedObject = null;
            else if (value && gameObject != null) UICamera.selectedObject = gameObject;
        }
    }
...
#if UNITY_IPHONE || UNITY_ANDROID || UNITY_WP8 || UNITY_BLACKBERRY
				if (Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.BB10Player ||
				    Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.WP8Player
//#if UNITY_WP8 
...
//#endif
	...
	void Update()
	{
        ...
        if (mKeyboard != null)
        {
            ...
            if (mText != val)
            {
                ...
                if (IsByLoudSpeaker() || IsByChatInfo())
                {
                    if (Utils.GetStrTextNum(val) > LoudSpeakerLogic.MAX_TEXTNUM)
                    {
                        canUpdate = false;
                    }
                }
 ...
 (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)))
    ...
    void Append(string input)
    {
        if (IsByLoudSpeaker() || IsByChatInfo())
        {
            if (Utils.GetStrTextNum(mText + input) > LoudSpeakerLogic.MAX_TEXTNUM)
            {
                return;
            }
        }
    ...
    bool IsByChatInfo()
    {
        return ChatInfoLogic.Instance() != null && 
            this == ChatInfoLogic.Instance().m_Input;
    }
}
